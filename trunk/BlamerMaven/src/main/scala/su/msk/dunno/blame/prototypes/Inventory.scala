...
class Inventory(val owner:Living) {
  ...
  def addItem(item:FieldObject) = {
    if(items.keys.size < 10) {
      ...
      FieldTracer.addTraceSecondToLast(owner.point, item)
  ...
  private lazy val inventory_screen = new ScageScreen("Inventory Screen") {
    ...
    key(KEY_1, onKeyDown = itemSelector = 1)
    key(KEY_2, onKeyDown = itemSelector = 2)
    key(KEY_3, onKeyDown = itemSelector = 3)
    key(KEY_4, onKeyDown = itemSelector = 4)
    key(KEY_5, onKeyDown = itemSelector = 5)
    key(KEY_6, onKeyDown = itemSelector = 6)
    key(KEY_7, onKeyDown = itemSelector = 7)
    key(KEY_8, onKeyDown = itemSelector = 8)
    key(KEY_9, onKeyDown = itemSelector = 9)
    key(KEY_0, onKeyDown = itemSelector = 10)

    key(KEY_NUMPAD1, onKeyDown = itemSelector = 1)
    key(KEY_NUMPAD2, onKeyDown = itemSelector = 2)
    key(KEY_NUMPAD3, onKeyDown = itemSelector = 3)
    key(KEY_NUMPAD4, onKeyDown = itemSelector = 4)
    key(KEY_NUMPAD5, onKeyDown = itemSelector = 5)
    key(KEY_NUMPAD6, onKeyDown = itemSelector = 6)
    key(KEY_NUMPAD7, onKeyDown = itemSelector = 7)
    key(KEY_NUMPAD8, onKeyDown = itemSelector = 8)
    key(KEY_NUMPAD9, onKeyDown = itemSelector = 9)
    key(KEY_NUMPAD0, onKeyDown = itemSelector = 10)
