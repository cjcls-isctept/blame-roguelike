...
object BottomMessages {
  ...
  val bottom_messages_height   = (font_size+3)*bottom_messages_capacity
  ...
  def showBottomMessages(skip_lines:Int) {
    var h = bottom_messages_height - (skip_lines + 1)*(font_size+2)
    if(skip_lines == 0 || skip_lines+bottom_messages.size <= bottom_messages_capacity)
      bottom_messages.foreach(message => {print(message, 10, h, WHITE); h -= (font_size+2)})
    else bottom_messages.dropRight(skip_lines).foreach(message => {print(message, 10, h, WHITE); h -= (font_size+2)})
  }
}
